...
class TextChunker:
    ...
    def _adaptive_chunk(
        ...
        """
        try:
            logger.debug(f"Starting adaptive chunking on text length: {len(text)}")
            chunks = []
            chunk_index = 0

            # First, split by paragraphs
            logger.debug("Splitting by paragraphs...")
            ...
            for i, para in enumerate(paragraphs):
                ...
                logger.debug(f"Processing paragraph {i+1}: length {len(para)}")
                ...
                if len(current_chunk) + len(para) > self.chunk_size:
                    ...
                    if len(current_chunk) >= self.min_chunk_size:
                        ...
                        chunks.append(chunk)
                        chunk_index += 1
                        logger.debug(f"Created chunk {chunk_index} (length {len(current_chunk)})")
                    ...
                    if len(para) > self.chunk_size:
                        logger.debug("Paragraph too large, splitting by sentences...")
            ...
            if len(current_chunk) >= self.min_chunk_size:
                ...
                chunks.append(chunk)
                logger.debug(f"Created final chunk {chunk_index+1} (length {len(current_chunk)})")

            logger.info(f"Created {len(chunks)} adaptive chunks from text of length {len(text)}")
            logger.debug(f"Adaptive chunking completed: {len(chunks)} chunks")
            return chunks

        except Exception as e:
            logger.error(f"Error in adaptive chunking: {e}, falling back to simple chunking")
