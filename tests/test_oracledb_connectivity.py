...
class TestOracleVectorStoreMocked:
    ...
    def test_connection_with_tls(self, mock_connect):
        ...
        assert call_kwargs['password'] == 'test_pass'  # pragma: allowlist secret
    ...
    def test_batch_insert_optimization(self, mock_connect):
        """Test that batch insert executes one INSERT per chunk"""
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_cursor.fetchone.return_value = [1]  # Table exists
        mock_conn.cursor.return_value = mock_cursor
        ...
        # Verify execute was called once per chunk (VECTOR binds require individual inserts)
