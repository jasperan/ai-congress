  ...
  return (
    <div className="min-h-screen">
      ...
      <nav className="sticky top-0 z-50 backdrop-blur-xl bg-background/80 border-b border-border">
        <div className="max-w-5xl mx-auto px-4 py-2.5 flex items-center gap-1.5 overflow-x-auto scrollbar-hide">
      ...
      <header id="top" className="relative overflow-hidden py-28 md:py-36">
        <div className="hero-glow bg-[#d4753a] top-[-200px] left-[10%]" />
        <div className="hero-glow bg-[#3aafbe] top-[-100px] right-[20%]" />
        <div className="max-w-5xl mx-auto px-4 relative z-10">
          <p className="font-mono text-xs text-muted-foreground tracking-[0.2em] uppercase mb-5">
            Interactive Explorer
          </p>
          <h1 className="text-4xl md:text-[3.5rem] font-bold tracking-[-0.03em] leading-[1.1] mb-7">
            Where{" "}
          ...
          <p className="text-sm text-muted-foreground mt-5 font-mono tracking-wide">
            Every concept below is interactive. Click, drag, simulate.
          </p>
        </div>
      </header>

      {/* ---- Table of Contents ---- */}
      <div id="main-content" className="max-w-5xl mx-auto px-4 mb-20">
        <div className="bg-card border border-border rounded-2xl p-8">
          <h2 className="text-sm font-mono text-muted-foreground uppercase tracking-[0.15em] mb-6">
            Chapters
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {tocItems.map((ch) => (
              <a
                key={ch.id}
                href={`#${ch.id}`}
                className={`group block p-5 rounded-xl border ${ch.border} bg-card transition-all duration-200 hover:bg-white/[0.02] hover:translate-y-[-2px]`}
              >
                <div className="flex items-baseline gap-3">
                  <span className={`font-mono text-xs ${ch.color} opacity-70`}>{ch.num}</span>
                  <h3 className="font-semibold text-sm group-hover:text-foreground transition-colors">{ch.title}</h3>
                </div>
                <p className="text-xs text-muted-foreground mt-1.5 ml-7 leading-relaxed">{ch.desc}</p>
              </a>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 prose-dark">
        ...
        <section className="reveal mb-16">
          ...
          <div className="bg-card border border-border rounded-2xl p-8 my-10">
            <h3 className="text-sm font-mono text-muted-foreground uppercase tracking-[0.15em] mb-6">Architecture at a glance</h3>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
              {[
                { label: "Swarm Modes", value: "4", color: "var(--color-ch1)" },
                { label: "Intelligence Improvements", value: "35", color: "var(--color-ch2)" },
                { label: "ACP Modules", value: "14", color: "var(--color-ch5)" },
                { label: "Reasoning Modes", value: "3", color: "var(--color-ch4)" },
                { label: "Simulation Phases", value: "6", color: "var(--color-ch9)" },
              ].map((stat) => (
                <div key={stat.label} className="text-center p-4 rounded-xl border border-border bg-background/50 transition-all duration-200 hover:border-border-strong">
      ...
      {/* ---- Footer ---- */}
      <footer className="max-w-5xl mx-auto px-4 py-10 border-t border-border">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            <span className="font-mono font-semibold text-foreground/70 tracking-tight">AI.congress</span>
            <span className="opacity-30">|</span>
            <span>Autonomous LLM multi-agent system</span>
          </div>
          <div className="flex items-center gap-4">
            <span>FastAPI + Svelte + Rust TUI + Ollama</span>
            <a
